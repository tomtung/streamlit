...
5. Compose PR (see @agent-knowledge/processes/pr-creation/writing-style.md)
...
- MUST follow @agent-knowledge/processes/pr-creation/writing-style.md (2-4 bullets max, omit obvious changes)
...
# Format: <verb> <what> <where> (≤50 chars) - see @agent-knowledge/processes/pr-creation/writing-style.md
...
2. **Analyze changes** to determine labels (see @agent-knowledge/processes/pr-creation/labeling-guide.md)

3. **Generate concise PR title** (≤80 chars, see @agent-knowledge/processes/pr-creation/writing-style.md)

4. **Compose PR description** using @.github/pull_request_template.md (2-4 bullets max, see @agent-knowledge/processes/pr-creation/writing-style.md)
...
6. **After user approval, create PR:**
   ...
   # If security assessment completed:
   gh pr create \
     --title "{Approved Title}" \
     --body-file pr_description.md \
     --base develop \
     --label "impact:{users|internal}" \
     --label "change:{type}" \
     --label "security-assessment-completed" \
     --draft

   # If security assessment NOT completed:
   gh pr create \
     --title "{Approved Title}" \
     --body-file pr_description.md \
     --base develop \
